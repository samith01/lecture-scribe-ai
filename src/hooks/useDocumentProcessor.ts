...
const SYSTEM_PROMPT = `You are an expert note-taker and document builder. 
Your job is to organize all key content from a full transcript into a clean, non-redundant, logically ordered document using this schema:
...
[
  {
    ...
    "children": [
      {
        "type": "bullet",
        "text": "Point under this section"
      },
      {
        "type": "subheading",
        "title": "Subtopic",
        "children": [
          { "type": "bullet", "text": "A point under the sub-topic" }
        ]
      }
    ]
  }
...
RULES:
- Output only this JSON tree with as many 'heading', 'subheading', and 'bullet' nodes as needed.
- Keep order and structure exactly as expressed in the lecture.
- DO NOT duplicate content, headings, or points. Merge info where relevant.
- Bold important technical terms using **double asterisks** in bullet text.
- Never output actions or edits. Only output the complete tree from scratch every time.
- Never use line indexes, never output animation instructions, do not mention keys or ids.
- If updating, fully regenerate (overwrite) the structure, preserving earlier doc content if still relevant.
- Never repeat identical sections or bullets.
...
export const useDocumentProcessor = ({
  ...
  const processTranscript = useCallback(
    async (fullTranscript: string) => {
      ...
      const newContent = fullTranscript.slice(
        lastProcessedTranscript.current.length
      );
      ...
      try {
        ...
        const userPrompt = `
        Here is the FULL TRANSCRIPT of the lecture so far:
${fullTranscript}

Here is the CURRENT DOCUMENT:
${currentMarkdown}

TASK: Update the document by re-organizing, merging, and adding all new and relevant content from the transcript (that is not already present), and respond ONLY with the full, updated JSON tree.
            ...
            result.edits.forEach((edit) => {
              if (edit.action === "add_heading" && edit.content) {
                ...
                if (targetHeading) {
                  ...
                  currentBulletKey.current = bullet.key;
                  console.log(
                    "Added default heading and bullet:",
                    edit.content
                  );
                }
